      ...
      <div className="grid grid-cols-12 h-screen w-screen px-56">
        <div className="col-span-3  pt-6">
          ...
          <div>
            <ul className="text-2xl  mt-4 font-semibold pr-4">
              {sidebarMenuItems.map(item=> <li key={item.title} className="flex justify-start items-center gap-4 hover:bg-gray-600 rounded-full p-4 w-fit cursor-pointer"> <span>{item.icon}</span> <span>{item.title}</span></li>)}
            </ul>
          </div>
        ...
        <div className="col-span-3 p-5">
          <div className="p-5 border bg-slate-700 rounded-lg">
            <h1 className="text-xl pb-3">New to Twitter?</h1>
